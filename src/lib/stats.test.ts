...
import type { GiveAwayLog, Purchase, Settings, SmokeLog } from "./types.ts";
...
test("assignSmokeCosts ignores packs bought after the cigarette (FIFO date guard)", () => {
  ...
  const lots = purchases
    ...
    .sort((a, b) => a.at - b.at)
    .map((p) => ({
      remaining: p.lines[0]!.packs * p.lines[0]!.cigsPerPack,
      per: p.cost / (p.lines[0]!.packs * p.lines[0]!.cigsPerPack),
    }));
...
test("inventoryRemaining subtracts smokes and give-aways", () => {
